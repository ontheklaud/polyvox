...
def fix_tacotron2_stock_pt(state_dict: dict, speakers_embedding_dim: int):
    ...
    for key in list_to_fix:
        state_dict_fix[key] = interpolate_tensor_shape_aware(state_dict_fix[key], speakers_embedding_dim)
...
def main():
    ...
    configs_aux_tacotron2: dict = {
        "n_speakers": 1, "speakers_embedding_dim": 16,
        "use_emotions": False, "n_emotions": 0,  "emotions_embedding_dim": 0,
    }
    ...
    if st.button("Generate"):
        if input_text.strip():
            ...
            with torch.no_grad():
                ...
                outputs = tacotron2_m.infer(sequences, speaker_id, use_ms=False)
                mel = outputs[1]
